...
**Handles are immutable parameter modifiers.** `apply_handles(processes, {handle: severity}, registry)` returns a *new* dict; every call names its registry, and the library has no default one. The demos' hallmarks of aging are `demos.models.hallmarks.HALLMARK_REGISTRY`, mapped onto the demo models. Severity is differentiable end-to-end.
...
- **Perturbation handle**: `hallsim.handles` (`Handle`, `ParameterMapping`, `apply_handles`, `with_handles`). A registry that names specific models is domain content and lives with them: `demos/models/hallmarks.py`. `hallsim.hallmarks.HALLMARK_INTENTS` describes the hallmarks in ontology terms and `suggest_registry` proposes a registry for a composite; the proposal is reviewed into a file, never resolved at apply time.
...
3. **Tolerance-sensitive** — the load-bearing check. Run at a loose and a tight tolerance. **If the trajectory changes materially, the result is solver-dependent and is not yet a result at the loose tolerance.** The action is to integrate at `ScreenReport.rtol_required`, not to discard the model: both runs finished and were finite, so the screen has measured a setting, not a defect. On an oscillator, read it twice — the measure is pointwise `max|y_loose - y_tight|`, so a limit cycle whose period shifts slightly accumulates phase until the two runs are half a period apart and the difference is the full amplitude on an otherwise identical orbit. Compare cycle averages or phase-aligned quantities before calling a cycling model solver-dependent.
...
`hallsim.intake.triage_sbml` / `triage_batch` is the cheap gate that runs first — SBML metadata plus one solve, no reviewer. It **rejects** only what will not import, has unsupported constructs, or produced no usable trajectory (`ScreenReport.blocking`, i.e. exploding). Everything else the screen finds is a **flag**: tolerance sensitivity with the tolerance to use, a state that left its domain, a vanishing trajectory, a non-finite gradient, a missing time unit, thin ontology coverage, or an IC that is not a rest state (`rest_residual` = ‖f(y₀)‖/‖y₀‖). Only `verdict.escalate` models are worth a reviewer's time.

Keep the reject list narrow on purpose. A screen finding that describes a run which *succeeded* is information about how to use the model, and folding it into the same boolean as "went to infinity" throws away both the model and the measurement that was just paid for. The tolerance check discarded the oscillators for oscillating until this was split.

**When the paper deposits its fitting data, run `intake.published_fit_chi2` first.** It answers "does this deposit still reproduce the fit its publication claims?", which is not the same question as "is this a good fit" — and it is what licenses every later finding. A deposit that reproduces its paper's χ² is the object the paper analysed, so a defect found afterwards belongs to the paper rather than to the deposition or to our import; a deposit that does not is a broken artefact and nothing found against it means anything. Keys may be a store path or a tuple of paths summed (a measured total over states the model keeps apart), with `scales` for deposited observable-scaling factors. `demos/dp14_published_fit.py` is the worked case: DallePezze 2014 returns **χ² 70.3706 over 127 points against a reported 70.4278 (0.08%)**, with the observable map read out of the SBML assignment rules rather than hand-listed. Most papers deposit no such data — that is a real limit on the check, and worth reporting as a finding about the field rather than hiding.
