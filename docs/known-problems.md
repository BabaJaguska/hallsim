...
- [ ] **P1.31 — Thirty-nine BioModels deposits diverge or raise under the Scheduler's default routing while an independent integrator keeps them bounded.** Found 2026-09-20/21 by the corpus census over both branches (`simulate census run`, rows in `outputs/2026-09-20_20-45-08/census.csv`, `framework_suspect` true): BIOMD0000000053 BIOMD0000000077 BIOMD0000000162 BIOMD0000000460 BIOMD0000000465 BIOMD0000000466 BIOMD0000000574 BIOMD0000000711 BIOMD0000000712 BIOMD0000000719 BIOMD0000000761 BIOMD0000000769 BIOMD0000000770 BIOMD0000000803 BIOMD0000000820 BIOMD0000000826 BIOMD0000000849 BIOMD0000000873 BIOMD0000000886 BIOMD0000000952 BIOMD0000000968 BIOMD0000000973 MODEL0848676877 MODEL0911270004 MODEL1210260004 MODEL1608250000 MODEL1806010001 MODEL1808310001 MODEL1812040001 MODEL1812040005 MODEL1907260001 MODEL1911150001 MODEL1912170004 MODEL1912170005 MODEL2003050002 MODEL2005070001 MODEL2201250001 MODEL5954483266 MODEL7909395757. Three symptoms — `EquinoxRuntimeError` from the implicit step, growth past 1000× and still rising, and NaN/Inf — on models the screen's reference stepper integrates to a bounded trajectory. The screen reports each as a framework issue rather than a model defect, so nothing is silent, but each is a published ODE the default path cannot run: 22 of them curated, 2.4% of the curated models that compile. Likely one cause per symptom: the stiffness verdict at `y0` routing a group explicit that turns stiff along the trajectory, and the chord root finder giving up where a fresh Newton would not. Start with Eftimie 2017 (BIOMD0000000769/770), whose reference peak is 10⁹, and Koo 2013 (465/466), which the 900 s retry also could not run.
...
- [ ] **P3.28 — Constructs the importer declines, counted over BioModels.** From the census, 2026-09-21, both branches: a species with no initial value (118 deposits, 1 curated — other simulators default it to zero, and adopting that convention with a warning lifts all of them), compartment volume varying in time (15 curated, 87 uncurated), `delay` in MathML (10), delayed events (3), `rateOf` (3), algebraic rules (3), symbolic stoichiometry (3), and event triggers or assignments that reference a symbol the translator does not resolve (10). The unset-initial-value convention and the varying compartment together are 220 deposits, most of the importer's remaining gap.
- [ ] **P3.29 — Twenty-three deposits do not finish the screen inside 900 s.** From the census, 2026-09-21: BIOMD0000000451 BIOMD0000000463 BIOMD0000000468 BIOMD0000000496 BIOMD0000000497 BIOMD0000000510 BIOMD0000000511 BIOMD0000000515 BIOMD0000000516 BIOMD0000000723 BIOMD0000000988 MODEL1112050000 MODEL1305010000 MODEL1603270000 MODEL1711210001 MODEL1711210002 MODEL1805150001 MODEL1812070002 MODEL2109110001 MODEL2211070001 MODEL3632127506 MODEL7519354389 MODEL9147232940. The Kerkhoven 2013 trypanosome glycolysis set and the Stanford 2013 yeast metabolic models are among them; each is a few hundred reactions with rational rate laws, so the cost is the sympy compile and the tight-tolerance reference solve rather than the size. A compile-time budget per model, or a screen that uses the model's own SED-ML span instead of ten native units, would tell timeouts that are the screen's from timeouts that are the model's.
