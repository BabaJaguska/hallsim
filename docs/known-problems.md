...
- [ ] **P3.33 — The only data reader is the GEO series matrix.** Measured by the data census of 2026-09-21 (`outputs/census-data/2026-09-21`): of 15,757 deposited time courses that a screened model carries a quantity of, 4,813 are GEO arrays the loader reads and 10,944 stop at a reader that does not exist: 8,161 GEO sequencing series (no counts file recognised among their supplementary names), 2,024 ArrayExpress processed tables, 433 MetaboLights assay (MAF) files, 263 PRIDE result files, 63 GEO counts files. A counts-file reader and a MAF reader would each be one route into `GeneExpressionDataset`'s shape or a `Collocation`; the proteomics result files vary by pipeline.
- [ ] **P3.34 — The mechanical design parser over-reads arms and under-reads time.** The sample-title parser (`hallsim.datasets.parse_design`) counts three or more arms on 167,131 of 359,741 datasets because sample-set and replicate tokens (`set1`, `c1`, `b`, `10`) survive as arm labels, so the perturbation labels it records on GEO series are mostly noise; and it finds a time course in 14,227 series' titles while the description supplies another 5,038, evidence the report keeps apart because the description count is loose. The plan's second tier, an agent read of the failures, is not in the framework; nothing is gated on arms, so the funnel stands, but the pair list's perturbation column does not.
