...
- [ ] **P3.43 — The path from a screened model to a scored contrast is walked by hand.** Recorded 2026-09-26 from the first agent trial, Birtwistle 2007 against its GEO cluster (`outputs/agent-trial/erbb-mapk/report.md`), in order of time cost after P0.92 and P0.93. (a) A platform whose identifiers do not resolve returns an empty frame with no error: GSE24391's HuEx `gene_assignment` column and GSE84096's accession-only platform both came back with zero genes after minutes of parsing; `choose_annotation` should raise on zero resolved symbols, parse HuEx assignments, and fall back to MyGene on an accession-only platform. (b) `compute_concordance` has no time-course form and reports an undefined Spearman as 0.0; the informative scores were a per-gene temporal rank and a cross-arm rank of peak responses, written in the trial's `score.py`, and belong in `gene_reporters` over `arm_deltas`-shaped input, with NaN where undefined. (c) `geo_series_urls` answers 404 on a SuperSeries (GSE6786); the family SOFT names its sub-series. (d) The census knows each deposit's arms and times but emits no per-sample `(arm, time) → GSM` map, so `sample_groups` for `from_series_matrix` was four regexes; `dataset_census` should emit it. (e) An arm the model cannot predict, an inhibitor alone on a model with no basal signalling, scores as a mismatch instead of being declared out of scope; the screen should report an observable with no activity in the control state and the scorer drop the arm with that reason. (f) The clock was inferred from a rate constant: the SBML declares no time unit, the importer assumed seconds, and nothing checked; `triage_sbml` should report a clock guess from the fastest first-order constant.
