...
- [ ] **P0.91 — Four registry entries and a docs page cite benchmark scripts
  ...
  *Fix:* commit them, or re-derive the numbers with something that is
  committed.
- [ ] **P0.92 — One Scheduler hands every later composite of the same structure the first one's initial state.** Found 2026-09-26 by the first agent trial and reproduced on a one-state decay: `Composite(initial={"cell/x": 1.0})` and then `Composite(initial={"cell/x": 3.0})`, run through one `Scheduler.run` with `y0` omitted, both return y(0) = 1.0 and y(4) = 0.135; a fresh Scheduler, or `y0=composite.initial_state_vec()`, returns y(0) = 3.0. The plan is cached on structure and the default `y0` is fixed when the plan is built rather than when it is run, so an arm sweep that sets ligand amounts through `initial` reports the first arm for every arm, at 0.1 s each after the first compile, which is what made it look like it was working. `with_handles` compounds it: it rebuilds the Composite without `initial=`, so a handle-applied arm loses its ligand override even with a fresh Scheduler. *Fixed 2026-09-26*: `run` takes the default `y0` from the composite it is handed on every call, `with_handles` carries `initial` across, and `test_each_composite_runs_from_its_own_initial_state` runs two initials through one Scheduler.
- [ ] **P0.93 — A linear-scale series matrix is read as log2, and the fold change is silently wrong.** Found 2026-09-26 by the first agent trial. `load_gene_expression` and `GeneExpressionDataset.from_series_matrix` take the values as deposited and `delta` subtracts them as log2. GEO series matrices from MAS5 and GCOS are linear (GSE6521 median 139, max 113,670; GSE6462 max 32,343; GSE6784 max 14,255; GSE24391 PLIER max 7,928) while RMA and SCAN matrices are log2 (GSE61364 spans −5 to 6, GSE84096 −1 to 4); four of the six matrices the trial fetched were linear and nothing warned. *Fixed 2026-09-26*: `load_gene_expression` passes the table through `as_log2`, which logs a linear table with a floor of 1 and a warning naming the deposit and its stated processing, and `GeneExpressionDataset.log_values` refuses a table whose values exceed the log2 ceiling.
...
- [ ] **P1.30 — A reporter can read a closed pool, and the guard that checks reporter mappings cannot see it.** Found 2026-09-13. UBB (a transcript) is mapped to Proctor 2007's free ubiquitin, `p07/Ub`. In that model free ubiquitin is consumed by E1 charging at a rate that depends only on ATP, E1 and the pool, and returns only when a ubiquitinated misfolded protein is deubiquitinated or degraded; there is no synthesis. Any perturbation that lowers misfolding leaves ubiquitin charged on E1/E2 and drains the free pool, so the composite reports UBB at −0.9 log2 under rapamycin against a measured −0.03 — a redistribution inside a closed pool read as expression. `reporter_wiring.validate_reporter_mappings` ran on every calibration and said, as a WARNING, `UBB ← p07/Ub: no MIRIAM annotation on the node, so its molecular kind can't be verified`; the deposit annotates neither `Ub` nor `MisP`, the warning went to a log nobody reads (the assistant filtered it out of every log it showed), and even annotated (UniProt P0CG47) the verdict would be "proxy: reads the gene's own protein", which is not the defect. The defect is structural: the observable belongs to a moiety with sinks and no source. `composite_moieties` does not surface it because the importer freezes Proctor's `degUb*` sinks as boundary, so the ubiquitin moiety has no exact law over integrated paths. *Fix:* (1) the verdicts now go into every run's `config.json` and the calibrate command prints them first, unfiltered; (2) add a structural verdict to the guard — an observable whose moiety (over the member's full stoichiometry, sinks included) has no production reaction from outside itself is a closed pool and cannot represent expression — using `composite_stoichiometry` with boundary rows kept as sinks; (3) drop UBB from the reporter set, or read the HSF1-driven load instead.

- [ ] **P1.32 — Complementary event triggers and equality-against-time triggers reject eleven curated deposits at triage.** Found 2026-09-20 by the census: eight deposits carry event pairs with complementary triggers sharing a boundary (geq/lt or gt/leq on one expression) and three trigger on an equality against time. Triage blocks them by design, but a curator ran every one of these, so the block is a statement about our event semantics (fire on a False→True crossing at a sync point) rather than about the deposit; a crossing-time root find, or treating a shared-boundary pair as one toggle, would run them.
...
- [ ] **P3.23 — A reaction-level sample is seconds of serial event loop.** Measured 2026-09-12 on the multi-hallmark composite with Proctor 2007 at reaction level (`demos/hallmark_levers.py`, 11-core M3 Pro): 3.0 s for one cell, 4.7 s for two, 5.8 s for four, 8.9 s for eight; ~6.9×10⁵ reaction events per cell over the 15-day run, ~4 µs an iteration. Direct SSA pays one loop iteration per event and, batched, evaluates every cell per iteration and runs each window to the slowest cell, so on the vectorised lane a population costs ~3 s + 0.7 s per cell and the lever page shows a stale mean field for that long after every pull; the growth with cell count is what (1) below removed, the per-chain floor is what remains. *What does not help here — measured before proposing it:* tau-leaping. At Proctor's settled operating point (ROS 14, day 3 of an exact run) the free E1 and E2 pools sit at 1 molecule, MisP at 4 and every MisP–ubiquitin conjugate at 0–4, so the Cao–Gillespie–Petzold step selection (ε 0.03 or 0.1, critical threshold 3 or 10 firings) allows a leap of 7–8 s that absorbs 0.5–0.8 events, while 57–68 % of all events are on the critical set and would be simulated exactly anyway. The model is small-number by construction — that is why it is stochastic — and leaping is for pools in the thousands. An earlier version of this entry proposed it as a hundredfold saving; it is not one. *What does help:* (1) **landed 2026-09-12** — independent members on host threads instead of one vectorised loop, as `Scheduler(batch_mode="auto"|"vectorized"|"threaded")` with `max_batch_workers`; `auto` takes the threaded lane for a batch carrying a stochastic process and leaves every other batch vectorised, and a traced call is always vectorised since threads are Python. It went in the Scheduler's batched lane rather than a demo-side pool, as this entry asked. On the 80-core host the lane is flat where the vectorised one is linear — 4 cells 11.9 s, 8 cells 12.2 s, 16 cells 12.1 s, 32 cells 12.8 s, against 18.7 / 28.9 / 39.1 / 58.1 s vectorised, with a single chain at 10.1 s — so 32 cells cost 1.27 chains rather than 5.8; 2.16x through `Scheduler.run` at 8 cells, and `demos/hallmark_levers.py` raised its default from 4 cells to 16. The lanes agree to 2.6e-17 of peak with every Gillespie count identical (only `p07/ROS` moves, and it is the continuous ROS drive). This caps at `ceil(cells / cores)` chains on a host with fewer cores than cells, and does nothing for the per-chain cost, which is (2). Earlier Mac figures: four one-cell loops on four threads 4.4 s against 6.9 s batched and 17.4 s sequential, eight on eight threads 5.5 s against 8.9 s. The shard_map and pmap attempts remain broken on this stack (P0.70); see `docs/benchmarks.md` section 6. (2) The per-iteration cost, measured through the Scheduler on one cell over 15 days (2026-09-12): 6.5 µs an event with the generated propensities; 2.3 µs with the propensity call replaced by a constant vector, so the evaluation is about two thirds of the iteration and the loop's own work (two draws, the categorical pick over 94 channels, the stoichiometry column, the save bookkeeping) the rest. A prototype that evaluates the 84 monomial laws as one 94×38 exponent-matrix product (exact to 4×10⁻¹⁶ against the generated laws, the nine x(x−1)/2 dimerisations and the driven synthesis coefficient handled) ran at 9.4 µs — slower: XLA fuses the generated scalar expressions well and the dense elementwise product is more work, not less. What is left inside the evaluation was measured 2026-09-12 and is not the lever either: in isolation the whole propensity call costs 0.27 µs against the 13 µs event it sits inside, of which `_constants(t)` is 26% and the species stack 0.6% — the stack is built once at trace time, not per event. Two dynamics-preserving swaps through the real loop, an inverse-CDF in place of `jax.random.choice` and one fused validity reduction in place of two, came to +3.9% and +3.3% against a 5% control arm, so neither is separable from noise. Branching the save-grid write was 3.8-4.0% slower and is in `docs/benchmarks.md` under rejected. Attribution by removal is unreliable here (the arms sum to 171%); the note there says how to measure it. (3) A slow-scale reduction of the ubiquitin charging cycle (partial equilibrium, Cao–Gillespie–Petzold 2005), which removes the E1/E2 firings — 52 % of one cell-day from the published initial state — rather than leaping over them; model-specific, and the same facility P3.5 asks for.
...
- [ ] **P3.27 — `single_process_composite` cannot build a model with no species.** Found 2026-09-20 by the census: Morris 2008 and 2009 (BIOMD0000000566, 567, protein aggregation fitted as rate rules on parameters) raise `ValueError: Need at least one array to stack` because the state vector is assembled from species only. A rate rule on a parameter is an integrated quantity; the composite should carry it as a state.
- [ ] **P3.28 — Constructs the importer declines, counted over BioModels.** After the 2026-09-21 re-screen, both branches: `delay` in MathML (10), delayed events (4), symbolic stoichiometry (3), `rateOf` (3), algebraic rules (3), and event triggers or assignments that reference a symbol the translator does not resolve (a handful). Forty-one deposits in all, down from 254: a species with no initial value and a time-varying compartment, which were the bulk of the gap, are translated. A parameter with no value is refused, as libRoadRunner refuses it, and counts as a deposit defect.
- [ ] **P3.31 — No variable-order implicit solver: a model that is very stiff at a tight tolerance is beyond the fixed-order ESDIRK ladder.** Found 2026-09-21 while closing P1.31. Eftimie 2017 (BIOMD0000000769/770, a logistic tumour saturating at 10⁹) solves on the ladder at rtol 1e-3 to libRoadRunner's peak within 0.4% and fails on every rung at rtol 1e-7, the screen's tight tolerance. After the 2026-09-21 re-screen nine deposits still do not finish the screen in 600 s — BIOMD0000000496/497 (Stanford 2013 yeast metabolism, 280 states), BIOMD0000000988, MODEL1001200000, MODEL1305010000 (which CVODE also refuses), MODEL1805150001, MODEL1812070002, MODEL2211070001, MODEL7519354389 — and Eftimie is a bounded reject. CVODE, a variable-order BDF, integrates most of these in milliseconds by dropping to low order on the stiff stretch. diffrax has no BDF; the options are a BDF written for diffrax, or a CVODE call behind a custom primitive with CVODES for the adjoint, and neither is small.
- [ ] **P3.32 — Four deposits still raise inside the importer.** After the 2026-09-21 fixes: MODEL1006230100 and MODEL1006230112 (Vinnakota 2010) and MODEL1908050003 (Benary 2015) raise `TypeError: Cannot convert complex to float` on a path other than the initial-value evaluation that P3.26 fixed, and MODEL2401110001 raises `Invalid NaN comparison` on a path other than function inlining. Each needs its traceback read; the census carries them as framework defects.
- [ ] **P3.33 — Most matched time courses still have no reader, and the gap is now classification rather than parsing.** *Partly closed 2026-09-25.* An RNA-seq counts reader exists: `read_counts_table` resolves an index of Entrez, Ensembl, symbol or accession identifiers through MyGene, sums counts per symbol, and converts to log2 CPM, so `GeneExpressionDataset.from_counts` feeds calibration unchanged. Verified end to end on GSE194123, where 39,376 Entrez rows resolved to 37,662 genes across nine samples and every SASP gene is present. Reclassifying the run lifts readable matched time courses only from 4,813 to 4,876, because `loader_of` sees a series' *supplementary file types* and NCBI's reprocessed counts are not among them. Measured over 120 of the 8,161 unreadable matched series: NCBI has reprocessed 18% overall, 35% of human and 3% of mouse, which extrapolates to about 1,500 more. Closing that needs a per-accession network check on survivors, roughly 8,000 requests, in the shape the `annotated` gate already uses. A MetaboLights reader landed the same day and closes the 433 assay files: `hallsim.metabolites` reads the assignment and ISA-Tab sample files through `metabolights-utils` and returns ChEBI-keyed contrasts, so `maf` joins the readable set and a metabolomics row now clears every gate. Verified on MTBLS1, MTBLS24 and MTBLS92. An mzTab reader landed the same day for PRIDE: `hallsim.proteins` reads the protein section through `pyteomics.mztab` and returns UniProt-keyed contrasts, verified on PXD060715. It is deliberately **not** in `READABLE`, because a tally of 281 PRIDE projects found only 6% deposit an mzTab (10% a MaxQuant `proteinGroups`, 7% a DIA-NN matrix, 9% identifications with no quantities, 60% a table of no fixed layout) and `_ebi_candidate` carries no file list, so no row says whether one exists. `ppx.find_project(acc).remote_files("*.mzTab")` answers it per accession, in the same network pass this entry already wants for NCBI's counts. The other proteomics formats belong to `directlfq`, which declares 46 of them and rolls up from ion level. Metabolomics Workbench closed the same day: `hallsim.metabolites.read_mwtab` parses mwTab with the repository's own `mwtab` package and resolves its PubChem, InChIKey and LIPID MAPS identifiers to ChEBI through UniChem, measuring 80% coverage on ST000001 against 44-53% for MetaboLights' deposited ChEBI annotation; `mwtab` is in `READABLE`. The remaining ~6,600 GEO series ship author-uploaded tables of no fixed layout, plus 2,024 ArrayExpress processed tables.
- [ ] **P3.37 — Plate and well coordinates still read as arm labels.** Left open by P3.34's fix. A token like `P06` or `A11` recurs across samples, so the frequency rule keeps it and a plate-based study reads as hundreds of arms: GSE314310, a ligand-stimulation atlas, parses as 460. Nothing is gated on arm count so the funnel stands, but the perturbation labels on those rows are noise. A positional rule would close it: a token whose distinct values number about as many as the samples is an index whatever its frequency.
- [ ] **P3.39 — EBI candidate descriptions are truncated before the time-course check reads them.** Found 2026-09-25. `_ebi_candidate` stores `description` cut to 400 characters, and `timed_evidence` falls back to `time_values(title + summary)` when neither the sample titles nor a declared factor shows a time course. A study that names its timepoints late in an abstract is therefore counted untimed across PRIDE, MetaboLights, Metabolomics Workbench and ArrayExpress. Same shape as the `raw.summary` truncation that manufactured 1,194 phantom parser regressions in the GEO census, so the fix is the same: keep the text the gate reads at full length and truncate only for display. Size of the undercount is unmeasured; re-running the EBI routes after the change measures it.
- [ ] **P3.40 — A deposit's loader is asserted from its modality where only its files can say.** Fixed for classification 2026-09-25, open for the file list itself. `loader_of` named a reader for ArrayExpress and PRIDE rows from `(source, modality, complete)`, because `_ebi_candidate` populates no file list. Measured over 80 matched ArrayExpress accessions: all ship an SDRF, 66% ship any data file, 34% ship none with their raw reads in ENA, and 8% have a counts-shaped name — the rest are per-sample raw arrays (764 `raw-data` text files, 378 `.cel`, plus `.idat`, `.gpr`, `.bam`, `.mtx`), which need RMA rather than a table read. Those rows now read `unchecked` and stay out of the readable count. The ArrayExpress and PRIDE file lists were fetched 2026-09-25, BioStudies for the one and the PRIDE API for the other: 115 of 1,314 matched ArrayExpress deposits ship a counts-shaped file and 2 of 263 PRIDE projects an mzTab, so on those two sources the item is closed. The same shape recurred on GEO with a file-type token in place of a modality: E-utilities lists a series' supplementary files as `TXT`, `CSV`, `XLSX`, `MTX`, never by name, so the counts-file rule could fire only on GEO's own `COUNTS` token and 51,453 matched RNA-seq series shipping an author table read `none`. *Closed 2026-09-26*: a sequencing series known only by tokens is `unchecked`, its names are fetched from the series' brief SOFT record (`geo_files`, throttled to NCBI's three requests a second), a counts-shaped name must also be a delimited table, and the rescreen selects what to fetch from a fresh screen rather than the stored row, which had kept an older rule's `pass` on 367 series. Over 57,589 matched RNA-seq series the names put 25,473 (44%) on the counts reader and the readable count from 30,200 to 55,186. What the rest ship is reader work under P3.33: 15,950 a `_RAW.tar` of per-sample files, 6,878 a table under a name that says nothing about counts (1,702 of them a differential-expression result), 5,285 a workbook only, 1,494 nothing beyond the reads in SRA.
- [ ] **P3.41 — The funnel counts deposits where the question is how much independent evidence exists.** Found 2026-09-25 while scoring aging clocks over OSDR brain studies. A repository lists one deposit per tissue, so one experiment arrives as several accessions over one set of organisms: OSD-563, OSD-564 and OSD-612 are the same ten RR-10 mice assayed as three brain regions, and OSD-561 and OSD-562 share 27 animals. Across eight studies, 136 samples came from 89 animals. Counting the deposits produced the only q < 0.05 in that analysis, from ten animals scored as thirty. `Design.n_subjects` and `_replicated` now count subjects where a deposit names them, and `shared_subjects` groups deposits that overlap — but the funnel itself still reports deposits, and `write_report` does not group them into experiments. Closing it means carrying subject identifiers through the row and reporting an experiment count beside the deposit count. ISA-Tab gives `Source Name`; GEO usually gives nothing, so the count is honest only where the repository states it.
- [ ] **P3.42 — Strain is not recorded, and two mouse studies on different backgrounds do not pool.** Found 2026-09-25. The census records organism only. Eight OSDR mouse-brain studies spanned four strains (BALB/c, C57BL/6J, C57BL/6NTac, B6129SF2/J) and between-study variance was 50-84% of total clock-score variance; restricting to one strain dropped I² to zero, so the heterogeneity was background and mission rather than biology. ISA-Tab carries `Characteristics[Strain]` and `Characteristics[Genotype]` and the reader already parses that frame; GEO rarely states it, which is itself worth recording as unknown rather than absent.
- [ ] **P3.38 — Crossing a study's declared factors makes every sample its own arm.** Found 2026-09-25 while reading MetaboLights sample files. `isa_design` labels an arm with the non-time factor values a sample declares, and MTBLS92 declares ten — BMI with 25 distinct values, Menopause, ER, Grade, Her2, N-stage, T-stage, Series, Timepoint, chemotherapy response — whose combination is unique for 419 of 447 samples. This is not the identifier case: `_drop_identifiers` prunes a token carried by one sample and here no token is, because each covariate value recurs. The study is observational and its patient covariates occupy the slot a designed experiment uses for arms, so no vocabulary distinguishes them. The rule that closes it is replication: an arm that holds one sample cannot be contrasted against anything, so factors should be added to a label only while every resulting group keeps at least two samples, cheapest factor first. Same shape as P3.37 and a candidate to fix once for both `parse_design` and `isa_design`. Nothing is gated on arm count, so the funnel stands and only the arm labels on such rows are noise.
- [ ] **P3.35 — The plan splits a small composite that a single implicit solve handles faster.** Measured in `docs/benchmarks.md` §7 (2026-09-11) and re-read 2026-09-22: the two-timescale chain at 72 states, a 64-state diffusion ring closed through an 8-state block relaxing at 10⁴, solves bare on Kvaerno5 in 47 ms and 139 steps, while the Scheduler's default lane at macro step 0.25 takes 108 ms at a relative error of 0.13, since 160 windows each restart two solvers and exchange state, and first-order Lie splitting across the loop sets the error. The same lane wins 2.6× at 264 states and 15× at 1,032, so the crossover lies between 72 and 264 states on this shape. The timescale clustering splits on rate ratio alone; it never asks whether the merged implicit solve is cheap, which a 72×72 factorisation is. The plan should estimate both costs, a merged implicit solve against the split at the chosen macro step, and take the merged fast path when the Jacobian is small enough that splitting only adds overhead; that is the stop rule's condition, a bare solve beating the framework, met on this shape.
- [ ] **P3.43 — The path from a screened model to a scored contrast is walked by hand.** Recorded 2026-09-26 from the first agent trial, Birtwistle 2007 against its GEO cluster (`outputs/agent-trial/erbb-mapk/report.md`), in order of time cost after P0.92 and P0.93. (a) *Fixed 2026-09-26 for the silent part*: a platform whose identifiers do not resolve now raises from `load_gene_expression` naming the platform's columns instead of returning an empty frame; GSE24391's HuEx `gene_assignment` column and GSE84096's accession-only platform still need their parsers. (b) *Fixed 2026-09-26*: `time_course_concordance` (per-gene rank over time and pooled, optional lag) and `peak_concordance` (cross-arm rank of peaks) live in `gene_reporters`, and an undefined Spearman is NaN everywhere, never 0.0. (c) `geo_series_urls` answers 404 on a SuperSeries (GSE6786); the family SOFT names its sub-series. (d) The census knows each deposit's arms and times but emits no per-sample `(arm, time) → GSM` map, so `sample_groups` for `from_series_matrix` was four regexes; `dataset_census` should emit it. (e) *Fixed 2026-09-26 on the scorer side*: `compute_concordance` returns `predicted_change=False` with NaN scores when the model is silent on a contrast, so an inhibitor-alone arm on a model without basal signalling is out of scope rather than a row of mismatches; the mechanism that gives such a model basal tone is composition, a `ClampEdge` standing ligand with its setpoint calibrated on those arms, and the transcript that trails a kinase is `transcript_pool`, a leaky `RunningIntegral` on the activity with its time constant calibrated — both primitives existed and neither was findable for the purpose, which P3.44's wiring fix and the trial skill address. (f) The clock was inferred from a rate constant: the SBML declares no time unit, the importer assumed seconds, and nothing checked; `triage_sbml` should report a clock guess from the fastest first-order constant.
- [ ] **P3.44 — The reporter wiring sees almost none of an imported model.** Reported 2026-09-26 by the first agent trial on Birtwistle 2007: 34 of 120 species carry ontology, the assignment-rule observables (`norm_Erk_star`, `norm_Akt_star`) and the active species `ERKstar` carry none although `MEKstar` beside it is annotated, `tf_observables` returned only NRG1, a ligand the CollecTRI-derived factor set lists, and `recommend_reporters` returned nothing, so the regulon route was closed and the trial bridged by hand through eight immediate-early genes read against `norm_Erk_star`. Fix: propagate ontology through assignment rules and the SBML `name` ("ERK*"), and filter the factor set by GO DNA-binding transcription factor activity.
