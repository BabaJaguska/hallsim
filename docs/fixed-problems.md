...
- [x] **P0.78 — With the chord root finder as the Scheduler's default, every
  ...
  non-finite or more than `growth_limit` (100; see P0.79) times larger, and so returns
...
- [x] **P3.24 — A single arm and absolute data could not be stated.** Filed and fixed 2026-09-19. Every arm had to be a `(condition, baseline)` pair under one global `normalization`, so a single time course was written as a pair of the condition with itself, and the loss's names (`model_lfc`, `Δ_data`) and calibration.md said fold change was the only comparison, although a `raw` mode existed, as log2 of the value with the model's units assumed. **Fixed:** each arm is an `Arm(condition, reference)`, the reference being `"t0"` (its own start, the default), another condition's name, or `None`, in which case the value itself is compared in the data's units through `GeneReporter.scale`, a fixed conversion. A plain condition name stands for `Arm(name)`. `arm_pairs` and `normalization` are gone from the problem (`arm_pairs` remains as a derived view for figure code), `model_lfc` is `model_readout`, and `GeneExpressionDataset.arm_deltas` takes the same arms so the data contrast follows the model's.

- [x] **P0.110 — Every rate mapping failed inside a jitted solve: the guard against a zero base value staged the concrete array.** Found 2026-09-20 by the generic page (`hallsim.view`) on its first composite: `ParameterMapping.value` checked `bool(jnp.all(jnp.asarray(base) == 0.0))` after confirming `base` was concrete, and inside any trace a `jnp` call on a concrete array is staged, so the `bool` hit a tracer. Only level mappings (which skip the guard) survived, which is why the hallmark-lever page's jitted solve had been broken since the guard landed on 2026-09-19 without its slow test running. **Fixed 2026-09-20:** the guard reads the array with numpy, which a trace does not stage. Regression: `tests/unit/test_handles_under_jit.py` applies a rate mapping under `jax.jit` and `jax.grad`.
- [x] **P0.109 — `screen_process(atol=...)` raised, and the screen called every molar-scale model dead.** Found 2026-09-20 by an outside agent screening Kowald 2006 (BIOMD0000000108), whose concentrations are in M: `atol` fell into `**sched_kwargs` while the solo run passed the production default by name, so the Scheduler saw two values; and the vanishing verdict was an absolute floor, `|y(t_end)| < 1e-9` on every state, which a live model at 1e-12 M cannot clear. **Fixed 2026-09-20:** `atol` is an argument of `screen_process`, held the same across the loose and tight runs and carried into the tunability check and the native reference integrator; a state is vanishing below `max(atol, 1e-6 × the trajectory's peak)`, nothing on its own scale or below what the solver resolves. Regression: `tests/unit/test_diagnostics.py::test_screen_takes_an_absolute_tolerance` screens a decay from 1e-12 M at `atol=1e-18` and expects a pass.
- [x] **P0.85 — `fit_neuralode_shooting` returns the last iterate, not the
  ...
  merely shipped. **Fixed 2026-09-20:** both NeuralODE trainers are wrappers over `CalibrationProblem`: the block is a `LearnedRef` fittable, each shooting window a batched condition from the observed state (`shooting_conditions`), derivative matching the `Collocation` term alone, and the Calibrator's best-iterate tracking applies. With a minibatched loss the iterates are ranked every `eval_every` steps on the whole objective, or on `eval_loss_fn`; the shooting wrapper ranks on one fixed draw of eight times its step batch, the fixed evaluation batch this entry asked for, and each stage warm-starts from the previous stage's best. A fixed draw of 512 collocation samples was tried first and ranked 9,000 iterates by luck (derivative block amplitude error 0.167 against 0.027); ranking on the whole objective gave 0.045, and the fixed 256-trajectory draw selected the same iterate. Regression guard, the hybrid demo's provenance (`outputs/multi_hallmark_hybrid/provenance_*.json`), held-out amplitude error: old loop 2026-09-14 0.0097 derivative / 0.0101 shooting; wrappers 2026-09-20 0.0167 / 0.0317. Three minibatch seeds of the derivative stage on the new path gave in-sample amplitude errors 0.045, 0.017 and 0.021 with the old loop's single 0.027 inside that range, so the derivative difference is seed spread; the shooting block is one run each side and its gap is not yet attributed. The derivative stage runs in 68 s against the old loop's 1,012 s, whose permutation draw over 4e5 samples cost 140 ms a step.
...
- [x] **P0.103 — A cached core runs its stochastic, discrete and event processes at the parameters the core was built with, so every arm of an eager sweep uses the first arm's.** Found 2026-09-12 from the lever page: at Loss of Proteostasis 1.0 (which sets p07's `k69`, proteasome activity, to 0) the mean field reached 1060 misfolded protein and 6321 aggregates while the population stayed at 6 and 3 — a lever that moved the deterministic arm and left the reaction-level arm flat. `Scheduler._build_core` closes over `stochastic_procs` / `discrete_procs` / `event_procs` and passes the composite separately; the continuous groups read the composite handed to the call, those three read the closure. A core built under a trace is deliberately not cached, so this could not show while a caller jitted around the run — `demos/hallmark_levers.py` did until `a3007f3` made the population eager to reach the threaded batch lane, at which point the core became cacheable and every setting after the first ran Proctor at the control preset's rates. Isolated: one `Scheduler` run at `k69=0.001` then `k69=0` returned the first run's 5 molecules for both, where a fresh `Scheduler` on the second returned 961. *Fix:* bind each process from the `composite` argument of `_run_scan_continuous` / `_run_scan_hybrid` rather than from the captured dict. Regression test `test_a_reused_scheduler_reads_reaction_rates_from_each_composite` runs a decay network at k=8 then k=0 through one `Scheduler` in both orders and asserts the second reflects its own rate; it fails against the captured binding. `tests/unit/test_reused_scheduler_hybrid_lane.py` does the same for the compiled discrete/event lane: a kick of 5 then 0 and a counter step of 1 then 0 through one `Scheduler`, both orders, against a fresh one. The class matters beyond this page: any eager parameter sweep, calibration loop or bifurcation scan that reuses a `Scheduler` over a composite carrying one of those three process kinds was silently running every arm at the first arm's parameters.
- [x] **P0.104 — `Composite.with_params` rejected the dotted address its own docstring shows.** Found 2026-09-18 by an outside reviewer following `docs/calibration.md`: `with_params({"p07.parameters.k1": k1})` raised `KeyError: No process 'p07.parameters'`. The key was split on its last dot (for nested `outer.inner` process names) while `write_param` needs the field as `parameters.k1`; `CalibrationProblem.with_overrides` split on the first dot, so the two routes disagreed on the same key. The reviewer finished the tutorial by replacing the parameters dict wholesale. **Fixed 2026-09-18:** `process.split_param_address` takes the longest process name the address starts with and hands the rest to `write_param`; `with_params`, `with_overrides` and the demo's `--fit process.key` addresses all go through it. The tutorial's rate-recovery block is self-contained and runs as written (k1 0.005 → 0.01199 against a true 0.012). Regression: `tests/unit/test_composition.py::TestParameterOverrides::test_with_params_reaches_a_parameters_entry`.
- [x] **P0.105 — `timescale` and `dt_step` were pytree leaves, so any jitted or differentiated function carrying a process pytree turned them into tracers and `auto_groups` failed inside it.** Found 2026-09-18 by an outside agent hand-rolling a `jax.grad` loss over a two-model composite: the sort in `auto_groups` raised on a tracer, with no message pointing at the cause. The fields were exempt from float-to-array coercion (kept as Python floats) but not static, which the code comment beside them already contradicted. **Fixed 2026-09-18:** both are `eqx.field(static=True)`, so a trace cannot reach them; `tests/unit/test_composition.py::test_grouping_fields_are_structure_not_pytree_leaves` groups a composite inside `jax.jit`.
- [x] **P0.35 — `Scheduler` + `expand_events` costs 223.80 s where one jitted
  ...
  **Fixed 2026-09-18, root cause found by profiling.** The per-macro-step cost was a compile per window: the eager macro loop handed the window bounds to the group solve as Python floats, and diffrax's own jit baked them into the traced program as constants, so every `(t, t_next)` was a new cache key and every macro step traced and compiled the solve again. The interpolated-coupling path baked the previous group's `(t0, t1)` into the next group's fill the same way. Measured on the Martinez-Guimera 2017 + Passos 2010 composite (two groups, three SBML events, a level edge): cold 4.4 s for 10 macro steps and 8.3 s for 20 before, 0.6 s for both after, warm 0.2 s either way; the single-group compiled scan was flat throughout, which is why the demo never showed it. Fix: `_solve_group`, `_solve_group_interpolated` and the interpolant handed between groups carry `jnp.asarray` bounds. The 2395x figure of 2026-09-04 predates the compiled hybrid lane and is superseded by these numbers.
- [x] **P0.106 — A composite with events could not be calibrated: the compiled lane's event record was materialised eagerly, and under the jitted loss that read a tracer.** Found 2026-09-18 by an outside agent whose freshly composed composite (two SBML imports carrying events, one level edge) crashed in `CalibrationProblem.fit` with a `TracerBoolConversionError`; the shipped demo composite has no events and never showed it. After the hybrid scan returns, `_execute` builds `EventRecord`s from the `event_fired` array with Python `bool(...)`, which a trace cannot do. **Fixed 2026-09-18:** under a trace the record is skipped (the deltas were applied inside the scan; nothing reads `result.events` inside a loss); eager runs still record every firing. The agent's reproducer (`repro_calibration_tracer_crash.py`, three fit steps on that composite) runs to completion. **Second occurrence, fixed 2026-09-19:** the same `bool(...)` sat in the eager macro-step loop's event branch, which is the path a multi-group composite with events takes under interpolated coupling (the scan lane takes events only under frozen coupling). The same agent hit it as soon as its composite gained a second timescale group. Under a trace the masked delta now lands every window, zero where nothing fired, and no record is kept; `test_an_event_composite_with_two_groups_runs_inside_a_trace` pins it.

- [x] **P0.107 — An imported model's translated events carried a JAX array inside a static field, so two imports of the same file were different structures.** Filed and fixed 2026-09-19. `SBMLEvent.time_scale` was a plain float field, which `__check_init__` coerces to a 0-d array, and the events sit in `SBMLProcess._events`, a static tuple: equinox warned "A JAX array is being set as static" on every import of a model with events, and the array hashed by identity, so the same model's structure differed across instances and every structure-keyed cache missed. Seen on BIOMD0000000287. **Fixed:** `time_scale` is a unit conversion, structure not value, and is static on the imported process and on `SBMLEvent`; `reconciled_to` and `expand_events` set it as a float through `write_param`'s static path. `test_an_import_with_events_is_one_structure` checks that no static field holds an array and that two imports share one treedef.

- [x] **P0.108 — An event-carrying model's protocol was invisible, so a wrong clock on it went unnoticed and the protocol was dropped whole when one input was driven.** Filed and fixed 2026-09-19 from an outside agent's run on BIOMD0000000287 (Passos 2010). The file declares no time unit; the import warns and assumes seconds. The agent passed `native_time_seconds=3600`, and the model ran 3600x slow: its irradiation events at 172800 and 172860 native units, 48 h and 48 h + 60 s on the seconds clock, sit 20 years out on the hours clock, and a 14-day composite covered 336 model-seconds. p21 moved by 4e-6 in both arms, and the agent concluded the damage axis was unidentifiable and four parameters confounded; on the seconds clock the native protocol takes p21 from 0.33 to 15.9 by day 4. The event translation was logged at INFO, which default logging hides, so the agent believed events were dropped at import; and driving an event-owned species refused with "drop the events", so the agent dropped all three, the day-8 `kp38ROS` switch included. **Fixed:** the import logs the protocol, every event's trigger time in native units and on the clock, and warns when it ends more than ten years after t=0 on the declared clock; triage flags the same; `provenance()` carries it as `events`; `with_species_input` on an event-set species drops only the events that write it, says so with their schedule, and refuses only an event that also writes something else; `without_events` takes names; `drive_pulse` drives a species too and takes its `dose` from that schedule (`native_input_level`), with `native_input_exposure` integrating it for the mismatch check.
- [x] **P3.25 — `process_from_sbml(<accession>)` could not fetch 400 of the 1,096 curated BioModels deposits.** Found 2026-09-20 by the corpus census (`simulate census run`): the download helper asked for `<accession>_url.xml`, which is the main file only on older deposits. From BIOMD0000000234 onward the main file often keeps the author's name (`GemcitabineCorrected.xml`, `Philipson2015.xml`, `Csikasz-Nagy2006.xml`) and the conventional name answers HTTP 400, so `simulate find` listed candidates that `.fetch()` and `triage_sbml` could not open. **Fixed 2026-09-20:** `sbml_import._fetch_biomodel_main` tries the convention, and on an HTTP error reads the deposit's record and fetches the file it names as main. Regression: `tests/unit/test_sbml_import.py::test_biomodel_download_falls_back_to_the_records_main_file`.
- [x] **P3.26 — The importer failed on seventeen deposits with an exception of its own.** Found 2026-09-20/21 by the census (`outputs/2026-09-20_20-45-08/census.csv`). Thirteen raise `TypeError: Cannot convert complex to float` (BIOMD0000000059 BIOMD0000000408 BIOMD0000000490 BIOMD0000000601 BIOMD0000000731 BIOMD0000000925 MODEL1006230100 MODEL1006230112 MODEL1405070000 MODEL1806150001 MODEL1908050003 MODEL2003190006 MODEL2007090001): a constant expression — an initial assignment or a parameter — evaluates to a complex number on the sympy path (a fractional power of a negative base or a square root of a negative intermediate, in every case a quantity the paper integrates as real). Three raise `TypeError: Invalid NaN comparison` (MODEL1807180003 MODEL1808150001 MODEL1910030001) and one `BadSignatureError: Duplicate symbol` (MODEL2003040001). libRoadRunner and COPASI run them. The fix for the first is to evaluate such constants with real-valued semantics (`sympy.re` where the imaginary part is zero, else an error naming the expression); the other two are one-line guards once the offending construct is read. **Fixed 2026-09-21:** the complex number was a stale value — the importer seeded a rule's target from its value attribute, so a later rule read 0/0 at t = 0; targets are no longer seeded, and a genuinely complex constant raises naming the expression. NaN is a symbol that lowers to `jnp.nan` at print time, so a comparison against it is false as IEEE says rather than a sympy error, and a lambda that binds a variable twice binds its first argument. Regressions: `tests/unit/test_sbml_core.py::test_a_rule_targets_value_attribute_is_not_read`, `::test_nan_in_a_comparison_imports_and_reads_false`, `::test_a_bound_variable_named_twice_binds_its_first_argument`.
- [x] **P3.30 — A species with no initial value and a compartment set by a rule were refused.** Counted by the census 2026-09-21: 118 and 102 deposits. **Fixed 2026-09-21:** an unset species or parameter defaults to zero with a warning, as libRoadRunner and COPASI do; a compartment a rate rule drives is integrated and one an assignment rule sets is assigned, values being amounts throughout, with the dilution term on a rate-ruled concentration. Conformance against libRoadRunner: Schaber 2012 and Zi 2011 to better than 1e-7. Regressions: `tests/unit/test_sbml_core.py::test_unset_initial_value_defaults_to_zero_with_a_warning`, `::test_varying_compartment_by_rate_rule_keeps_amounts`, `::test_varying_compartment_by_assignment_rule`.
- [x] **P1.31 — Thirty-nine BioModels deposits diverged or raised under the Scheduler's default routing while an independent integrator kept them bounded.** Found 2026-09-20/21 by the corpus census. **Fixed 2026-09-21**, after each of the 39 was run against libRoadRunner and under explicit-only HallSim at both screen tolerances (`outputs/census/2026-09-20_20-45-08/triage_divergences*.jsonl`); they were four things. Twelve had a Jacobian that is not finite at y0 (a square root or fractional power of a state that starts at zero): the stiffness analysis raised, the fallback routed every group implicit, and Newton could not use that Jacobian either — a non-finite Jacobian at y0 now routes explicit (`tests/unit/test_multiscale.py::test_a_non_finite_jacobian_at_y0_routes_explicit_and_runs`). Thirteen were the screen's growth heuristic calling a model that grows past 1000× and is still rising "exploding", then reading the reference's identical peak as a framework fault — agreed growth is now the `growing` advisory (`tests/unit/test_diagnostics.py::test_agreed_growth_is_growth_not_divergence`). One failed only at the loose tolerance, which the screen now reports as a tolerance finding rather than a failed model. Eight turned stiff along their trajectory after a non-stiff verdict at y0 (Eftimie 2017's logistic tumour), and the Scheduler now runs a solver ladder: a group whose explicit solve fails is re-run on the implicit solver, a stiff group runs the fifth-order implicit solver under `LADDER_STEP_BUDGET` and moves to `Kvaerno3` with diffrax's per-step chord if it runs out, and the verdict is remembered (`::test_a_model_that_turns_stiff_is_rerouted_to_the_implicit_solver`). Five the reference could not run either. What the ladder cannot reach is P3.31.
- [x] **P3.29 — Twenty-three deposits did not finish the screen inside 900 s.** Found 2026-09-21 by the census. Every one simulates its ten native units in libRoadRunner in under 0.13 s, so the cost was HallSim's. **Fixed 2026-09-21** for the mechanisms found: Carbo 2013 and its kind were the non-finite Jacobian of P1.31 sending a Newton to grind; Kerkhoven 2013's set was the fifth-order implicit solver with optimistix's per-stage chord at the screen's loose tolerance rejecting steps in cascade — 500 000 steps and failing where diffrax's per-step chord takes 1 528 — which the ladder's third rung now takes over after a hundred thousand steps (119 s to the right answer, against never). Timeouts that remain after the 2026-09-21 re-screen are the fixed-order limit, P3.31.
- [x] **P0.111 — The intake screen cost fifty seconds to a quarter of an hour per model where the solve is seconds.** Found 2026-09-21 when a re-screen of 409 deposits paced at three hours against libRoadRunner's milliseconds; the halt condition in CLAUDE.md. Measured: the tunability gradient through the solve was 56 s on Noble 1998 and the growth check's reference integrator (`odeint` at rtol 1e-12, five million steps) ran past thirteen minutes on Heldt 2012, while the Scheduler's warm solve of the same models is 0.1 s. **Fixed 2026-09-21:** the tunability probe differentiates the compiled core's rate function at sampled states (56 s → 0.75 s; the through-the-solve check stays behind `check_tunability="solve"`); the reference is a bounded bare-diffrax implicit solve; routing verdicts are shared across Scheduler instances (`_RUNG_MEMORY`) so a screen's tight and loose runs and a census's per-deposit instances do not each climb the ladder; the explicit rung has a step budget. Full triage: Heldt 13.5 s, Kerkhoven 62 s, Eftimie 79 s to a bounded reject. Regressions: `tests/unit/test_diagnostics.py::test_field_level_tunability_catches_a_non_finite_parameter_derivative`.
- [x] **P0.112 — A calibration problem could not be built for an arm with no observations.** Found 2026-09-21 by CI on a fresh checkout, which has no series matrix: `CalibrationProblem` stacked an empty column list for each arm's data block and raised, so the multi-hallmark demo's problem could not be constructed before its dataset was fetched. **Fixed 2026-09-21:** an arm without timepoints gets a block with no timepoints, `(n_readouts, 0[, n_batch])`. Regression: `tests/unit/test_cli.py::test_the_problem_builds_without_the_dataset`.
- [x] **P0.113 — The routing memory shared across Scheduler instances was keyed by structure alone.** Found 2026-09-22 by CI: `verify_plan` reported no moved verdict after a million-fold rate change, and a batched run inherited whichever verdict the previous test left behind. The memory that lets a screen's tight and loose runs and a census's per-deposit instances share one ladder climb had dropped the parameter digest from its key, the mistake `_param_digest`'s own docstring names. **Fixed 2026-09-22:** the memory is keyed by structure and parameter digest, with the structure's latest eager verdict kept separately for a traced run that cannot measure its own; an instance that takes a verdict from the memory records it as its own, so its traced runs follow the arm it was warmed on; the test suite clears the memory between tests. Regressions: `tests/unit/test_multiscale.py::test_verify_plan_reports_a_moved_verdict`, `::test_reused_scheduler_does_not_inherit_a_stiffness_verdict`.
- [x] **P3.34 — The mechanical design parser over-read arms, so it under-read time.** *Fixed 2026-09-25.* Diagnosed by cross-checking against an independent screen (Biomni's, 3,434 pairs over 811 GEO series): both runs covered all 811, and they disagreed on 180, all in the direction of Biomni calling something a time course that we did not. The cause was that a token appearing in exactly one sample title — an accession, a well, a plate, an animal id — survived into the arm label, so every sample became its own arm and no arm held more than one timepoint. GSE303646 read as 56 arms of 1 timepoint where it is 4 arms (young/old × control/bleomycin) of 5. `_drop_identifiers` now prunes tokens carried by a single sample before the common head and tail are stripped; frequency decides, so it needs no vocabulary and works on an unseen cohort. Time courses recognised from sample titles rise from 14,227 to 15,608 across the 229,210 GEO series, with no losses on that route. Correct rejections are unchanged: the kidney-organoid biobank and the ligand atlas still fail, and on those Biomni was wrong. Regression: `tests/unit/test_datasets.py`.
- [x] **P3.36 — A stored row could not reproduce its own verdict.** *Found and fixed 2026-09-25 while measuring P3.34.* `screen_dataset` capped `raw.summary` at 600 characters, and 32% of the 359,741 rows hit that cap, but the `timed` gate's text route reads the description. So `census-data rescreen`, whose whole purpose is re-judging a run offline, silently disagreed with the run that wrote it: 1,194 rows lost their verdict for that reason alone and it read as a regression in the parser. The summary is stored untruncated now.
