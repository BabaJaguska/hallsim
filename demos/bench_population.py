...
PLOT_PATH = cfg.get(
    "plot", str(outdir("bench_population") / "bench_population.png")
)
...
log(
    f"hallsim: {len(species)} species, n_vars={n_vars}, "
...
rh = sched.run(
    comp, t_span=(0.0, T_END), macro_dt=1.0, save_dt=T_END / (N_SAVE - 1)
)
rr.reset()
...
for N in NS:
    ...
    log(
        f"  N={N:>5}  hallsim(1 T4)={th:7.3f}s  tellurium(1)={ts:7.3f}s  "
...
ax1.loglog(
    arr[:, 0], arr[:, 1], "o-", label="hallsim (1 Tesla T4, batched f64)"
)
...
ax1.set_title("Population simulation: wall time vs N")
ax1.legend()
...
ax2.set_ylabel("hallsim speedup (tellurium / hallsim)")
ax2.set_title("hallsim speedup over tellurium")
ax2.legend()
