...
def main():
    ...
    print(
        f"batched solve: {ys.shape}  in {time.time()-t0:.1f}s  "
        f"finite={np.isfinite(ys).all()}"
    )

    idx = {k: i for i, k in enumerate(keys)}
    reporters = {lbl: p for lbl, p in REPORTERS.items() if p in idx}
...
def _plot(dist, cvs, out, N):
    ...
    fig, axes = plt.subplots(
        1, 2, figsize=(13, 4.4), gridspec_kw={"width_ratios": [1.3, 1]}
    )
    # per-cell reporter distributions, normalized to population median
    ax = axes[0]
    data = [dist[lbl] / np.median(dist[lbl]) for lbl in labels]
    ...
    for i, lbl in enumerate(labels, 1):
        ax.scatter(
            np.full(len(dist[lbl]), i)
            + np.random.uniform(-0.06, 0.06, len(dist[lbl])),
            dist[lbl] / np.median(dist[lbl]),
            s=4,
            color="#0e7490",
            alpha=0.25,
            zorder=3,
    ...
    ax.set_title(
        f"Every hallmark reporter is now a distribution\n"
        f"({N} cells, one batched composite solve)",
        fontsize=10,
    )
    # CV bars
    ax = axes[1]
    y = np.arange(len(labels))
    ax.barh(y, [cvs[lbl] for lbl in labels], color="#0e7490", alpha=0.8)
    for i, lbl in enumerate(labels):
        ax.text(cvs[lbl] + 0.5, i, f"{cvs[lbl]:.0f}%", va="center", fontsize=9)
    ax.set_yticks(y)
