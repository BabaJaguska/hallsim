...
def run(psi):
    ...
    comp = Composite(
        processes={"gz06": gz}, topology={}, semantic_validation=False
    )
    r = Scheduler().run(
        comp, t_span=(0.0, T_END), macro_dt=T_END, save_dt=0.05
    )
    return (
        np.asarray(r.ts),
        np.asarray(r.get("gz06/x")),
...
def main():
    ...
    for ax, (psi, tag) in zip(
        axes,
        [
            (1.0, "psi = 1.0   (full damage / DDIS)"),
    ...
    fig.suptitle(
        "Geva-Zatorsky 2006 — p53-Mdm2 oscillator  [BIOMD157]",
        fontsize=12,
        x=0.02,
        ha="left",
        fontweight="bold",
